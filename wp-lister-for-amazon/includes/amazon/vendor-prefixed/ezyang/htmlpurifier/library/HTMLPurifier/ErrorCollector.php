 ...
 */
class WPLab_Amazon_HTMLPurifier_ErrorCollector
{
    ...
    /**
     * @type WPLab_Amazon_HTMLPurifier_Language
     */
    protected $locale;
    ...
    public function send($severity, $msg)
    {
        ...
        $new_struct = new WPLab_Amazon_HTMLPurifier_ErrorStruct();
        $new_struct->type = WPLab_Amazon_HTMLPurifier_ErrorStruct::TOKEN;
        if ($token) {
        ...
        if (!empty($attr)) {
            $struct = $struct->getChild(WPLab_Amazon_HTMLPurifier_ErrorStruct::ATTR, $attr);
            if (!$struct->value) {
                $struct->value = array($attr, 'PUT VALUE HERE');
            }
        }
        if (!empty($cssprop)) {
            $struct = $struct->getChild(WPLab_Amazon_HTMLPurifier_ErrorStruct::CSSPROP, $cssprop);
            if (!$struct->value) {
     ...
     * @param WPLab_Amazon_HTMLPurifier_Config $config Configuration, vital for HTML output nature
    ...
    public function getHTMLFormatted($config, $errors = null)
    {
        ...
        $this->generator = new WPLab_Amazon_HTMLPurifier_Generator($config, $this->context);
        if ($errors === null) {
            $errors = $this->errors;
        }
