...
class WPLab_Amazon_HTMLPurifier_Injector_RemoveEmpty extends WPLab_Amazon_HTMLPurifier_Injector
{
    /**
    ...
    public function prepare($config, $context)
    {
        ...
        foreach ($this->exclude as $key => $attrs) {
            if (!is_array($attrs)) {
                // HACK, see WPLab_Amazon_HTMLPurifier/Printer/ConfigForm.php
    ...
    public function handleElement(&$token)
    {
        ...
        for ($i = count($this->inputZipper->back) - 1; $i >= 0; $i--, $deleted++) {
            ...
            if ($next instanceof WPLab_Amazon_HTMLPurifier_Token_Text) {
        ...
        if (!$next || ($next instanceof WPLab_Amazon_HTMLPurifier_Token_End && $next->name == $token->name)) {
            ...
            for ($b = 0, $c = count($this->inputZipper->front); $b < $c; $b++) {
                ...
                if ($prev instanceof WPLab_Amazon_HTMLPurifier_Token_Text && $prev->is_whitespace) {
                    continue;
                }
                break;
            }
