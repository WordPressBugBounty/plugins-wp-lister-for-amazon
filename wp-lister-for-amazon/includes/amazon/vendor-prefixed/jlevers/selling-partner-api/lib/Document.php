...
use WPLab\Amazon\GuzzleHttp\Psr7\Header;
use WPLab\Amazon\GuzzleHttp\Psr7\InflateStream;
use WPLab\Amazon\GuzzleHttp\Psr7\Utils;
use WPLab\Amazon\GuzzleHttp\RequestOptions;
use WPLab\Amazon\PhpOffice\PhpSpreadsheet\IOFactory;
use WPLab\Amazon\Psr\Http\Message\StreamInterface;
use RuntimeException;

use WPLab\Amazon\SellingPartnerApi\Model\FeedsV20210630\CreateFeedDocumentResponse;
use WPLab\Amazon\SellingPartnerApi\Model\FeedsV20210630\FeedDocument;
use WPLab\Amazon\SellingPartnerApi\Model\ReportsV20210630\ReportDocument;
...
class Document
{
     ...
     * @param ?\WPLab\Amazon\GuzzleHttp\Client $client  The Guzzle client to use. If not provided, a new one will be created.
     */
    public function __construct(
        ...
        if (!in_array($this->contentType, array_values($validContentTypes), true)) {
            ...
            foreach ($validContentTypes as $name => $value) {
                $readableContentTypes[] = "WPLab\Amazon\SellingPartnerApi\ContentType::{$name} ($value)";
            }
        ...
        if (!($this->contentType === ContentType::XLSX || $this->contentType === ContentType::PDF)) {
            if (!is_null($encoding) && !in_array(strtoupper($encoding), mb_list_encodings(), true)) {
                ...
                if ($response->hasHeader('content-type')) {
                    ...
                    $parsedHeader = \WPLab\Amazon\GuzzleHttp\Psr7\Header::parse($httpContentType);
